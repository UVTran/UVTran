...
def main(args):
    ...
    fna = fii*400
    inid = 0
    for fiId in range(fii,fii+1000):
        testflag = False
        curnum = 1
        ...
        for uvid in range(curnum):
            ...
            for poi in range(len(pcu[uvi[uuvid]])):
                ...
                norm = normalize(cross_product(skl1[1][0], skl1[0][1]))
                pono = []
                curx.append(skl1[0][0][0])
                cury.append(skl1[0][0][1])
                curz.append(skl1[0][0][2])
            ...
            newx = np.zeros((201, 201), dtype=float)
            newy = np.zeros((201, 201), dtype=float)
            newz = np.zeros((201, 201), dtype=float)
            ...
            ax1.plot(curx, cury, curz, label="oricurve", color='red')
            ax1.plot_surface(newx, newy, newz, color='lightblue', alpha=0.8, label="newsurf")
            # ax1.scatter(ncurx, ncury, ncurz, label="oricurve", color='red')
            ...
            # 保留网格
            ax1.grid(True)
            ax1.set_title('old 3D Surface Plot')
