...
def get_args_parser():
    ...
    parser.add_argument('--f', default=20, type=int)
...
def main(args):
    ...
    if not os.path.exists("../g1data/test/input/"):
        os.makedirs("../g1data/test/input/")
    if not os.path.exists("../g1data/test/output/"):
        os.makedirs("../g1data/test/output/")
    if not os.path.exists("../g1data/test/voutput/"):
        os.makedirs("../g1data/test/voutput/")

    if not os.path.exists("../g1data/input/"):
        os.makedirs("../g1data/input/")
    if not os.path.exists("../g1data/output/"):
        os.makedirs("../g1data/output/")
    if not os.path.exists("../g1data/voutput/"):
        os.makedirs("../g1data/voutput/")
    ...
    midsrf = BSpline.Surface()
    midsrf.degree_u = 3
    midsrf.degree_v = 3
    midsrf.ctrlpts_size_u = 8
    midsrf.ctrlpts_size_v = 8
    ...
    fna = fii*400
    inid = 0
    for fiId in range(fii,fii+5000):
        testflag = False
        curnum = 5
        ...
        for uvid in range(curnum):
            ...
            for poi in range(len(pcu[uvi[uuvid]])):
                ...
                norm = normalize(cross_product(skl1[1][0], skl1[0][1]))
                pono = []
                pono.append(skl1[0][0][0])
                pono.append(skl1[0][0][1])
                pono.append(skl1[0][0][2])
                pono.append(norm[0])
                pono.append(norm[1])
                pono.append(norm[2])
            ...
            pointtensor = torch.tensor(rpo).reshape(-1)
            voxet = torch.tensor(vo)
            ...
            if testflag:
                torch.save(geolist2, '../g1data/test/input/' + str(fna+inid) + '.pt')
                torch.save(pointknot, '../g1data/test/output/' + str(fna+inid) + '.pt')
                torch.save(voxet, '../g1data/test/voutput/' + str(fna+inid) + '.pt')
            else:
                torch.save(geolist2, '../g1data/input/' + str(fna+inid) + '.pt')
                torch.save(pointknot, '../g1data/output/' + str(fna+inid) + '.pt')
                torch.save(voxet, '../g1data/voutput/' + str(fna+inid) + '.pt')
